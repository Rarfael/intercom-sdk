...
It exposes the API as capitalised, semantic **Entities** — e.g. `client:Contact()` — each with the same small set of operations (`list`, `load`, `create`, `update`, `remove`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
```lua
local contact, err = client:Contact():load({ id = "example_id" })
if err then error(err) end
print(contact)
```
...
```lua
-- Create
local created, err = client:Contact():create({ created_at = 1, email = "example_email" })
if err then error(err) end
