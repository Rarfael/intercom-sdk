...
It exposes the API as capitalised, semantic **Entities** — e.g. `client.Contact(nil)` — each with the same small set of operations (`List`, `Load`, `Create`, `Update`, `Remove`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
func main() {
    ...
    // List contact records — the value is the array of records itself.
    ...
    // Load a single contact — the value is the loaded record.
    contact, err := client.Contact(nil).Load(map[string]any{"id": "example_id"}, nil)
    if err != nil {
        panic(err)
    }
    fmt.Println(contact)
    ...
    // Create a contact.
    created, err := client.Contact(nil).Create(map[string]any{"created_at": 1, "email": "example_email"}, nil)
    if err != nil {
        panic(err)
    }
    fmt.Println(created)
    ...
    // Update a contact.
    updated, err := client.Contact(nil).Update(map[string]any{"id": "example_id", "created_at": 1, "email": "example_email"}, nil)
    if err != nil {
        panic(err)
    }
    fmt.Println(updated)
    ...
    // Remove a contact.
    removed, err := client.Contact(nil).Remove(map[string]any{"id": "example_id"}, nil)
    if err != nil {
        panic(err)
    }
    fmt.Println(removed)
...
slice):
    ...
    contact, err := client.Contact(nil).List(map[string]any{/* fields */}, nil)
    if err != nil { /* handle */ }
    // contact is the returned record
