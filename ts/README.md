# Intercom TypeScript SDK



The TypeScript SDK for the Intercom API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.Contact()` — each with a small set of operations (`list`, `load`, `create`, `update`, `remove`)
...
`list()` resolves to an array of Contact ENTITIES — every operation
...
```ts
try {
  const contact = await client.Contact().load({ id: 'example_id' })
  console.log(contact)
} catch (err) {
...
```ts
// Create — returns the created Contact ENTITY (.data() for the record)
