...
(0, node_test_1.describe)('ContactDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-contact', async (t) => {
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-contact', setup.live))
            return;
...
function directSetup(mockres) {
    ...
    if (live) {
        const client = new __1.IntercomSDK({
            apikey: env.INTERCOM_APIKEY,
        });
        let idmap = env['INTERCOM_TEST_CONTACT_ENTID'];
