...
const TEST_SEED = { "entity": { "contact": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+IntercomSDK\b/.test(code) ||
        /\bIntercomSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+IntercomSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/IntercomSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
