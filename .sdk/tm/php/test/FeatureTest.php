...
class FtCtrl extends IntercomControl
{
...
class FtClient
{
    ...
    private IntercomUtility $utility;

    public function __construct(IntercomUtility $utility, array $options)
    {
    ...
    public function get_utility(): IntercomUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (IntercomContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public IntercomContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (IntercomContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = IntercomConfig::make_feature($name);
    ...
    private static function buildUrl(IntercomSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof IntercomError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new IntercomResponse($fetched);
        }
    ...
    private function fail(IntercomContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (IntercomContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
