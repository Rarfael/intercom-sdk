...
The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->Contact()` — with named operations (`list`/`load`/`create`/`update`/`remove`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
### 2. List contact records

```php
try {
    // list() returns an array of Contact records — iterate directly.
...
try {
    ...
    $contact = $client->Contact()->load(["id" => "example_id"]);
    print_r($contact);
} catch (\Throwable $err) {
    echo "Error: " . $err->getMessage();
}
...
```php
// create() returns the ENTITY — call data_get() for the created Contact record.
$created = $client->Contact()->create(["created_at" => 1, "email" => "example_email"]);

// Update — index the record via data_get() ($created->data_get()["id"]).
$client->Contact()->update(["id" => $created->data_get()["id"], "created_at" => 1, "email" => "example_email"]);

// Remove
$client->Contact()->remove(["id" => $created->data_get()["id"]]);
```
...
$client = IntercomSDK::test([
    "entity" => ["contact" => ["test01" => ["id" => "test01"]]],
]);
