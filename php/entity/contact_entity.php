...
class ContactEntity
{
    ...
    {
        if ($args) {
            $this->_data = IntercomHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = IntercomHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *
     * @param ContactLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed ContactLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Contact|array The loaded Contact as an assoc-array at the
     *   SDK boundary; throws IntercomError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = IntercomHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *   the SDK boundary; throws IntercomError on failure (item-5 convention).
     */
     ...
     *
     * @param ContactUpdateData|array|null $reqdata Body data as an assoc-array;
     *   a typed ContactUpdateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Contact|array The updated Contact as an assoc-array at the
     *   SDK boundary; throws IntercomError on failure (item-5 convention).
     */
    ...
    /**
     * Remove an Contact matching the given criteria.
     *
     * @param ContactRemoveMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; ContactRemoveMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Contact|array The removed Contact as an assoc-array at the
     *   SDK boundary; throws IntercomError on failure (item-5 convention).
     */
