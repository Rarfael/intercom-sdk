...
{
    private static function utility(): IntercomUtility
    {
        return new IntercomUtility();
    }
    ...
    private static function ctx(array $over = []): IntercomContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new IntercomContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new IntercomOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof IntercomError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new IntercomOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = IntercomMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new IntercomOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = IntercomMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new IntercomOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = IntercomMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new IntercomSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = IntercomMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new IntercomResponse([]),
            'result' => new IntercomResult([]),
        ]);
        [, $err] = IntercomMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new IntercomSpec([]),
            'response' => null,
            'result' => new IntercomResult([]),
        ]);
        [, $err] = IntercomMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new IntercomSpec(['step' => 's']),
            'response' => new IntercomResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new IntercomResult([]),
        ]);
        [, $err] = IntercomMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new IntercomSpec(['step' => 's']),
            'response' => new IntercomResponse(self::resp(200, ['v' => 1])),
            'result' => new IntercomResult([]),
        ]);
        [, $err] = IntercomMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new IntercomSpec(['step' => 's']),
            'response' => new IntercomResponse(self::resp(200, ['v' => 2])),
            'result' => new IntercomResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        IntercomMakeResponse::call($ctx);
    ...
    {
        $preset = new IntercomResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new IntercomSpec([]),
            'response' => new IntercomResponse([]),
            'result' => new IntercomResult([]),
        ...
        [$got, $err] = IntercomMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new IntercomResult([])]);
        [, $err] = IntercomMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new IntercomSpec([]), 'result' => null]);
        [, $err] = IntercomMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new IntercomSpec(['step' => 's']),
            'result' => new IntercomResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new IntercomOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = IntercomMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new IntercomSpec(['step' => 's']),
            'result' => new IntercomResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new IntercomOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = IntercomMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new IntercomResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new IntercomSpec([]),
            'result' => new IntercomResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = IntercomMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = IntercomMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new IntercomError('boom', 'boom');
        $utility->fetcher = function (IntercomContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new IntercomSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = IntercomMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (IntercomContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = IntercomMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(IntercomResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (IntercomContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new IntercomSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = IntercomMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new IntercomResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new IntercomSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = IntercomMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = IntercomMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = IntercomMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new IntercomResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, IntercomDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new IntercomResult(['ok' => false])]);
        $this->expectException(IntercomError::class);
        IntercomDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new IntercomResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', IntercomMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        IntercomMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new IntercomBaseFeature();
        $b = new IntercomBaseFeature();
        IntercomFeatureAdd::call($ctx, $a);
        IntercomFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): IntercomBaseFeature
    {
        $f = new IntercomBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        IntercomFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): IntercomContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new IntercomSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = IntercomPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new IntercomResponse(['headers' => null]),
            'result' => new IntercomResult([]),
        ]);
        IntercomResultHeaders::call($ctx);
