...
class IntercomMakeError
{
    public static function call(?IntercomContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new IntercomContext([], null);
        }
        $op = $ctx->op ?? new IntercomOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new IntercomResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof IntercomError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new IntercomError('', $msg, $ctx);
