...
class IntercomTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            IntercomTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = IntercomTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = IntercomTestRunner.getenv("INTERCOM_TEST_LIVE")
        override = IntercomTestRunner.getenv("INTERCOM_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = IntercomTestRunner.getenv(key)
        ...
        explain = IntercomTestRunner.getenv("INTERCOM_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["INTERCOM_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if IntercomTestRunner._test_control is not None:
            return IntercomTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                IntercomTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            IntercomTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = IntercomTestRunner.load_test_control()
...
def load_env_local():
    IntercomTestRunner.load_env_local()


def env_override(m):
    return IntercomTestRunner.env_override(m)


def entity_data(v):
    return IntercomTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return IntercomTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return IntercomTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return IntercomTestRunner.load_test_control()


def live_delay_ms():
    return IntercomTestRunner.live_delay_ms()
